...
LATEST_APK=$(ls -t *.apk 2>/dev/null | head -n1 || true)
if [ -n "$LATEST_APK" ] && [ "$LATEST_APK" != "app-release-v${VERSION}.apk" ]; then
  mv "$LATEST_APK" "app-release-v${VERSION}.apk"
  echo "=> Build renomeado com sucesso para: app-release-v${VERSION}.apk"
fi
