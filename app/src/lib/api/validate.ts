...
export function parsePatchPerfilBody(body: unknown): {
  ...
  if (record.syncAutoEnabled !== undefined) {
    if (typeof record.syncAutoEnabled !== "boolean") {
      throw validationError("syncAutoEnabled deve ser booleano.");
    }
    result.syncAutoEnabled = record.syncAutoEnabled;
  }

  if (record.syncIntervalMinutes !== undefined) {
    const value = Number(record.syncIntervalMinutes);
    if (![15, 30, 60, 120].includes(value)) {
      throw validationError("Intervalo inválido. Use 15, 30, 60 ou 120 minutos.");
    }
    result.syncIntervalMinutes = value as 15 | 30 | 60 | 120;
  }

  if (
    result.syncAutoEnabled === undefined &&
    result.syncIntervalMinutes === undefined
  ) {
    throw validationError("Nenhuma preferência informada.");
  }
