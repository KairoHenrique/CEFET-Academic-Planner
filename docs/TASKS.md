...
> **Próximo oficial:** **#10 — Bloco 4 · F28** (site mobile). **F25–F27** `[@]` (skeletons, transições, favicon/título — push jul/2026, aguardando aprovação). **#9 fechado:** **B36/B37** `[x]` (alertas no sino, aprovado jul/2026) · **F24 descartado** (banners removidos — alertas ficam só no sino) · demais #9 `[x]`. Bloco **2f (B72a–e)** `[x]`. **Fix jul/2026:** login rápido normaliza CPF no `sync-readiness` (CPF formatado não força mais re-sync de 1º acesso).
...
### #10 — Bloco 4 · Polimento UX + site mobile `🟡 3/4`

> **Executar antes do mobile (#8).** Quem não instalar o app Android usa o site no celular (**F28**).

- [@] **FRONT:** F25 *(loading skeletons — todas as telas)* — `SubjectDetailSkeleton` estruturado; demais telas já cobertas (dashboard, disciplinas, calendário, mapa, integralização, simulador/grade)
- [@] **FRONT:** F26 *(transições de página entre rotas)* — fade-in keyed por `pathname` nas telas do aluno (`.route-transition`, respeita `prefers-reduced-motion`)
- [@] **FRONT:** F27 *(favicon + título na aba)* — favicon já existente + `title` template `%s · ACME HUB` + `metadata` por página
...
| **#10** | 4 | Polimento UX + site mobile (**F28**) | 🟡 **F25–F27** `[@]` (push jul/2026) · próximo **F28** | 3/4 |
...
**Próximo:** **#10 — Bloco 4 · F28** (site mobile). **F25–F27** `[@]` (push jul/2026, aguardando aprovação) · **B36/B37** `[x]` (sino) · **F24 descartado**. Bloco **2f/B72** e **B35/F20** `[x]` (push jul/2026).
...
- [@] **FRONT:** F25 → F26 → F27 *(push jul/2026, aguardando aprovação)* · [ ] F28
...
| #10 | 4 — Polimento + site mobile | 🟡 **F25–F27** `[@]` · próximo **F28** | 3 / 4 |
...
> **Próximo (produto):** **#10 — Bloco 4 · F28** (site mobile). **F25–F27** `[@]` (push jul/2026, aguardando aprovação) · **B36/B37** `[x]` (sino) · **F24 descartado**. Bloco **2f/B72** e **F20** `[x]` (push jul/2026).
...
- [@] Adicionar animações de transição entre páginas — fade-in keyed por `pathname` nas telas do aluno (`.route-transition`, `prefers-reduced-motion`) (**F26**)
- [@] Adicionar loading skeletons em todas as telas — dashboard, disciplinas (lista + `SubjectDetailSkeleton` estruturado), calendário, mapa, integralização, grade/simulador (**F25**)
- [x] Responsividade básica (breakpoints mobile/tablet/desktop)
- [@] Favicon e título na aba — favicon `favicon.ico`/`icon.png` · `title` template `%s · ACME HUB` + `metadata` por página (**F27**)
...
11. **Próximo passo:** **#10 — Bloco 4 · F28** (site mobile). **F25–F27** `[@]` (skeletons, transições de rota, favicon/título — push jul/2026, aguardando aprovação). **#9 fechado:** **B36/B37** `[x]` (alertas no sino, aprovado jul/2026) · **F24 descartado** (banners removidos — só sino) · demais `[x]`. Ops sync = PC home server (`npm run worker:home` + `npm run worker:tunnel` + secrets CF; dev scraper `SIGAA_BROWSER_CHANNEL=chrome`).
