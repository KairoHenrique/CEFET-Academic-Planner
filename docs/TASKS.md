...
**Tasks em `[@]` (jul/2026):** *(nenhuma — B56/O3 aprovadas jul/2026)*.
...
> **Próximo oficial:** **O1** (deploy Cloudflare + cron ping). Ver [`docs/plan/6a-supabase-plan.md`](./plan/6a-supabase-plan.md).
...
> **Progresso:** Bloco 1 ✅ · **Bloco 2a/2b** ✅ · **Bloco 6a** — **PLAN** ✅ · **B39** `[x]` · **B41–B43** `[@]` · próximo **O1**.
...
| **#4** | 6a | Supabase + deploy global | **B41–B43** `[@]` · próximo **O1** | 5/8 |
...
| B41 | Back | Seed PPC global | 61 disciplinas + requisitos · `npm run db:seed-ppc` | [@] |
| B42 | Back | Client Supabase | `@supabase/supabase-js` + adapter PG (`PLANNER_DATABASE=postgres`) | [@] |
| B43 | Back | Migrar APIs | dashboard · disciplinas · sync stub cloud | [@] |
...
> **Próximo:** **O1** — deploy Cloudflare + cron ping.
...
#### B43 — Migrar APIs (dashboard · disciplinas · sync stub) `[@]`
...
11. **Próximo passo:** **O1** (deploy Cloudflare + cron ping Supabase) — **B41–B43** `[@]` no remoto (dev postgres validado localmente).
