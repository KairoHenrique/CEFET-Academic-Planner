...
### #8 — Bloco 8 · Mobile Android (Expo Go) `🟡 1/16`
...
> **Progresso:** **M1** `[x]` (scaffold Expo SDK 54). **Local:** **M2** `[%]` — `@acme/api-contracts` em `packages/api-contracts` (commit local).

- [x] **SETUP:** M1 *(projeto Expo TypeScript — `mobile/` · **Android only** · **SDK 54** p/ Expo Go da Play Store)*
- [%] **SHARED:** M2 *(tipos/contratos compartilhados — `packages/api-contracts` · `@acme/api-contracts`)*
...
| **#8** | 8 | Mobile Android (Expo Go) | **M1** `[x]` · **M2** `[%]` · paridade site **v1.0** + push + sessão · **sem Play/App Store** | 1/16 |
...
**Próximo:** **#8 — Mobile Android** — **M1** `[x]` · **M2** `[%]` (contratos `packages/api-contracts`). Site **v1.0.0**. **F25–F28** `[x]`.
...
> **Progresso:** **M1** `[x]`. **Local:** **M2** `[%]` (`@acme/api-contracts`).

- [x] **SETUP:** M1 *(Expo `mobile/` · Android only · SDK 54)*
- [%] **SETUP/SHARED:** M2 *(tipos `@acme/api-contracts`)* → M3 → M4 *(sessão · auth)*
...
| #8 | 8 — Mobile Android | 🟡 **M1** `[x]` · **M2** `[%]` · paridade + push + sessão · sem lojas | 1 / 16 |
...
> **Próximo (produto):** **#8 — Mobile Android** — **M1** `[x]` · **M2** `[%]` · depois M3+ (paridade v1.0 + push + sessão). Site desktop **v1.0.0**. **F25–F28** `[x]`.
...
| M2 | Shared | Tipos | Contratos API — `packages/api-contracts` (`@acme/api-contracts`) | [%] |
...
- [%] **M2** — tipos compartilhados `@acme/api-contracts` (`packages/api-contracts`) — aguardando push
- [ ] **M3–M4** — sessão SecureStore + auth/gate (login fica até logout)
...
11. **Próximo passo:** aguardar push/aprovação de **M2** `[%]` (`packages/api-contracts` · `@acme/api-contracts`). **M1** `[x]`. Site **v1.0.0**. Bloco 8 `1/16`. **F28** `[x]`.
